...
def eval_helper_fn(tf_env, actor_net, prob_dropout, cutoff=10,
                   num_eval_episodes=5, stochastic_z=False, prefix='',
                   global_step=None):
  ...
  def rollout(network_state):
    ...
    for t in range(1000):
      ...
      b = tf.cond(ts.is_last(),true_fn= true_fn,false_fn=false_fn)
      if b:
        break
  ...
  for i in range(num_eval_episodes):
    print("in eval_dropout episode",i)
    network_state = rollout(network_state)
