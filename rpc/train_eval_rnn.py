...
def train_eval(
  ...
  with tf.compat.v2.summary.record_if(
    ...
    tf_agent = rpc_agent_rnn.RpAgent(
        time_step_spec,
        action_spec,
    ...
    if use_recurrent_actor:  # default from sac/train_eval_rnn.py
      ...
      def _filter_invalid_transition(trajectories, unused_arg1):
        return tf.reduce_all(~trajectories.is_boundary()[:-1])
      tf_agent._as_transition = data_converter.AsTransition(tf_agent.data_context, squeeze_time_dim=False)
    ...
    def train_step():
      ...
      input_trajs = experience.observation[:,:10]
      input_actions = experience.action[:,:10]
      obs_next = experience.observation[:,10]
      prior = predictor_net((input_trajs,
                             input_actions), training=False)
      z_next = encoder_net(obs_next, training=False)
