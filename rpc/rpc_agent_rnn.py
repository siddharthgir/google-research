...
class RpAgent(sac_agent.SacAgent):
  ...
  def _train(self, experience, weights):
    ...
    proper_time_steps,proper_actions,proper_next_time_steps = time_steps, actions, next_time_steps
    time_steps = TimeStep(time_steps.step_type[:,-1],time_steps.reward[:,-1],time_steps.discount[:,-1],time_steps.observation[:,-1])
    next_time_steps = TimeStep(next_time_steps.step_type[:,-1],next_time_steps.reward[:,-1],next_time_steps.discount[:,-1],next_time_steps.observation[:,-1])
    actions = actions[:,-1]
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      ...
      actor_loss = self._actor_loss_weight*self.actor_loss(
          proper_time_steps, proper_actions, proper_next_time_steps, weights=weights)
  ...
  def actor_loss(self,
    ...
    """
    pred_time_steps,pred_actions,pred_next_time_steps = time_steps, actions, next_time_steps

    prev_time_steps = TimeStep(pred_time_steps.step_type[:,-1],pred_time_steps.reward[:,-1],pred_time_steps.discount[:,-1],pred_time_steps.observation[:,-1])
    time_steps = TimeStep(pred_next_time_steps.step_type[:,-1],pred_next_time_steps.reward[:,-1],pred_next_time_steps.discount[:,-1],pred_next_time_steps.observation[:,-1])
    ...
    with tf.name_scope('actor_loss'):
      ...
      prior = self._actor_network._predictor((pred_time_steps.observation, pred_actions),  # pylint: disable=protected-access
                                             training=True)
